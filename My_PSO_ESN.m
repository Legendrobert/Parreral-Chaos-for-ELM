...
%% 构造训练样本、预测样本
matrix=[];          %每一行为一个训练样本，200个输入一个输出，共900行
...
U=matrix(1:M,1:K);  %每一行为一组输入，k个输入，共800行
y=matrix(1:M,K+1);  %每一行为一组输出，1个输出，共800行
Ustar=matrix(M+1:M+testlength,1:K);     %每一行为一组输入，k个输入，共100行
ystar=matrix(M+1:M+testlength,K+1);     %每一行为一组输出，1个输出，共100行
...
Wfb=rand(N,L);              %输出层与内部状态量连接权重，即输出层对状态层的反馈值
